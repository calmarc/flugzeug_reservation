...
if (isset($_POST['submit']))
{
  ...
  if (mail ($email, $subject, $txt, implode("\r\n",$headers)))
  {
	...
	write_status_message($mysqli, "[Passwort]", "System", "Recovery: Link wurde an &lt;{$email}&gt; <span style='color: green;'>geschickt</span>");
    $status_txt = "Ein Wiederherstellungs-Link wurde an &lt;{$email}&gt; geschickt.";
  }
  else
  {
	write_status_message($mysqli, "[Passwort]", "System", "Recovery: Link konnte <span style='color: red;'>nicht</span> an &lt;{$email}&gt; gesendet werden.");
